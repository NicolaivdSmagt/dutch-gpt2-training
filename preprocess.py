#!/usr/bin/env python3
"""
Dutch GPT-2 Training: Preprocessing Script

This script processes the BramVanroy/wikipedia_culturax_dutch dataset for GPT-2 training.
It downloads the dataset, tokenizes the text, and creates training chunks of fixed length.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Preprocess Dutch Wikipedia dataset for GPT-2 training")
    ...
    parser.add_argument(
        ...
        help="Size of the dataset to use (1M, 10M, 100M, 1B, 10B tokens)"
    )
...
def main():
    ...
    # Step 6: Create chunks of size args.chunk_size
    def create_chunks(examples):
        """Creates chunks of specified size from tokenized examples."""
        ...
        for i in range(len(examples['input_ids'])):
            ...
            for j in range(0, len(input_ids) - chunk_size + 1, chunk_size):
                all_input_ids.append(input_ids[j:j + chunk_size])
                all_attention_masks.append(attention_mask[j:j + chunk_size])
